...
# ============================================================
# Task 13B: JAX anisotropic extended-source benchmark
# ============================================================
...
# Volume radial grid
N_R = 41
r_unit = jnp.linspace(0.0, 1.0, N_R)

# Scan ranges
chi_shell = jnp.array([0.42, 0.46, 0.50, 0.54, 0.58], dtype=jnp.float32)
chi_volume = jnp.array([1.00, 1.10, 1.20, 1.30, 1.40], dtype=jnp.float32)
betas = jnp.array([-0.8, -0.4, 0.0, 0.4, 0.8], dtype=jnp.float32)
...
def build_point_baseline_intensity(harmonics, theta_grid, phi_grid):
    ...
    Returns cube shape (Nh, Ntheta, Nphi)
    """
    theta_obs, phi_obs = jnp.meshgrid(theta_grid, phi_grid, indexing="ij")
...
# mu(theta_obs, phi_obs, theta_src, phi_src)
MU = (
...
def shell_form_factor_anisotropic_single(k, weight_src, b):
    """
    Returns F(theta_obs, phi_obs) for one harmonic.
    """
    norm = jnp.sum(weight_src * sin_ts) * dtheta_src * dphi_src
    phase = jnp.exp(-1j * k * b * MU)  # (Ntheta_obs, Nphi_obs, Ntheta_src, Nphi_src)
    integ = jnp.sum(weight_src[None, None, :, :] * phase * sin_ts[None, None, :, :], axis=(2, 3))
    return integ * dtheta_src * dphi_src / norm


@jax.jit
def volume_form_factor_anisotropic_single(k, weight_ang, r_grid):
    """
    Returns F(theta_obs, phi_obs) for one harmonic.
    """
    norm_ang = jnp.sum(weight_ang * sin_ts) * dtheta_src * dphi_src
    dr = r_grid[1] - r_grid[0]
    norm_rad = jnp.sum(r_grid**2) * dr
    norm = norm_ang * norm_rad

    # shape (Nr, Ntheta_obs, Nphi_obs, Ntheta_src, Nphi_src)
    phase = jnp.exp(-1j * k * r_grid[:, None, None, None, None] * MU[None, :, :, :, :])
    ang_integ = jnp.sum(weight_ang[None, None, None, :, :] * phase * sin_ts[None, None, None, :, :], axis=(3, 4))
    # ang_integ shape (Nr, Ntheta_obs, Nphi_obs)

    weighted = (r_grid[:, None, None] ** 2) * ang_integ
    total = jnp.sum(weighted, axis=0) * dr
    return total / norm
...
def detector_score(intensity_cube, theta_grid, theta_min, theta_max, phi_index):
    mask = ((theta_grid >= theta_min) & (theta_grid <= theta_max)).astype(jnp.float32)  # (Ntheta,)
...
def full_3d_score(intensity_cube, theta_grid):
    sin_theta = jnp.sin(theta_grid)[:, None]
...
def save_profile(theta_grid_np, point_profile, ext_profile, title, fname):
    ...
    plt.plot(theta_grid_np, ext_profile, label="best anisotropic source")
...
# Family runners
# ----------------------------
def run_family(kind, chi_values, point_cube, Sdet_point, Sff_point):
    ...
    for ic, chi in enumerate(np.array(chi_values)):
        for ib, beta in enumerate(np.array(betas)):
            ...
            if kind == "shell":
                ...
                r_grid = chi * r_unit
                form_factors = jax.vmap(lambda n: volume_form_factor_anisotropic_single(n, weight, r_grid))(HARMONICS)
            else:
...
def robustness_check(kind, chi0, beta0, point_cube, Sdet_point, Sff_point):
    chis = np.array([
        chi0 - ROBUST_DELTA_CHI,
        chi0 - 0.5 * ROBUST_DELTA_CHI,
        chi0,
        chi0 + 0.5 * ROBUST_DELTA_CHI,
        chi0 + ROBUST_DELTA_CHI,
    ], dtype=np.float32)
...
# ----------------------------
def main():
    print("Starting Task 13B anisotropic extended-source scan (JAX)...", flush=True)
    ...
    shell_Rdet, shell_Rff, shell_Q, best_shell = run_family("shell", chi_shell, point_cube, Sdet_point, Sff_point)
    volume_Rdet, volume_Rff, volume_Q, best_volume = run_family("volume", chi_volume, point_cube, Sdet_point, Sff_point)
    ...
    # Heatmaps
    save_heatmap(shell_Rdet, np.array(betas), np.array(chi_shell), "Task 13B shell: $R_{\\det}$", r"$R_{\det}$", "task13B_shell_Rdet.png")
    save_heatmap(shell_Rff, np.array(betas), np.array(chi_shell), "Task 13B shell: $R_{\\mathrm{ff}}^{3D}$", r"$R_{\mathrm{ff}}^{3D}$", "task13B_shell_Rff3D.png")
    save_heatmap(shell_Q, np.array(betas), np.array(chi_shell), "Task 13B shell: $Q=R_{\\det}/R_{\\mathrm{ff}}^{3D}$", r"$Q$", "task13B_shell_Q.png")

    save_heatmap(volume_Rdet, np.array(betas), np.array(chi_volume), "Task 13B volume: $R_{\\det}$", r"$R_{\det}$", "task13B_volume_Rdet.png")
    save_heatmap(volume_Rff, np.array(betas), np.array(chi_volume), "Task 13B volume: $R_{\\mathrm{ff}}^{3D}$", r"$R_{\mathrm{ff}}^{3D}$", "task13B_volume_Rff3D.png")
    save_heatmap(volume_Q, np.array(betas), np.array(chi_volume), "Task 13B volume: $Q=R_{\\det}/R_{\\mathrm{ff}}^{3D}$", r"$Q$", "task13B_volume_Q.png")
    ...
    # Robustness
    shell_chis, shell_rdet_rob, shell_rff_rob = robustness_check("shell", best_shell["chi"], best_shell["beta"], point_cube, Sdet_point, Sff_point)
    volume_chis, volume_rdet_rob, volume_rff_rob = robustness_check("volume", best_volume["chi"], best_volume["beta"], point_cube, Sdet_point, Sff_point)

    save_robustness_plot(shell_chis, shell_rdet_rob, shell_rff_rob, "Task 13B shell robustness", "task13B_shell_robustness.png")
    save_robustness_plot(volume_chis, volume_rdet_rob, volume_rff_rob, "Task 13B volume robustness", "task13B_volume_robustness.png")
